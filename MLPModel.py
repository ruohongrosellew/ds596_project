...
class HillActivation(nn.Module):
    ...
    def forward(self, x):
        x = torch.relu(x)
        return (x ** self.n) / (self.K ** self.n + x ** self.n + 1e-8)
...
def get_activation(name):
    if name == "relu":
        ...
        return HillActivation()
    else:
        raise ValueError(f"Unknown activation {name}")

#Add different nonlinear activation funcitons (relu, gelu, sily, swish)
...
#Edit this block to take in the new activation functions (layer and forward)
class MLPBlock(TimestepBlock):
    """
    Basic MLP block with an optional timestep embedding.
    """

    def __init__(self, input_dim, output_dim, time_embed_dim=None, latent_dim = None, activation = "silu"):
        super().__init__()
        #gated layer
...
#Update this to take in activation 
class MLPModel(nn.Module):
    ...
    def __init__(self, 
        ...
        if use_encoder: 
            self.encoder = EncoderMLPModel(gene_size,self.hidden_sizes, self.num_classes, use_drug_structure, self.drug_dimension, comb_num, activation=activation)
        ...
        if self.use_encoder: 
            ...
            for _ in range(num_layers):
                layers.append(MLPBlock(hidden_sizes, hidden_sizes, time_embed_dim, latent_dim, activation))
            self.mlp_blocks = TimestepEmbedSequential(*layers)
        else:
            layers = []
            for _ in range(num_layers):
                layers.append(MLPBlock(hidden_sizes, hidden_sizes, time_embed_dim, activation))
            self.mlp_blocks = TimestepEmbedSequential(*layers)
...
class EncoderMLPModel(nn.Module):
    ...
    def __init__(self, input_size, hidden_sizes, num_classes=None, use_drug_structure=False, drug_dimension=1024,comb_num=1,output_size=60, dropout=0.1, use_fp16=False, activation = "relu"):
