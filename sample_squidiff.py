...
class sampler:
    ...
    def interp_with_slerp(self, z_sem_origin=None, gene_size=None, direction=None, scale=1, add_noise_term=True, t=0.5):
        """SLERP interpolation"""

        z_sem_origin_np = z_sem_origin.detach().cpu().numpy()
        z0 = z_sem_origin_np.mean(axis=0)
        z1 = z0 + direction.detach().cpu().numpy() * scale

        # convert to torch
        z0 = torch.tensor(z0, dtype=torch.float32, device='cuda')
        z1 = torch.tensor(z1, dtype=torch.float32, device='cuda')

        # normalize
        z0_norm = z0 / (z0.norm() + 1e-8)
        z1_norm = z1 / (z1.norm() + 1e-8)

        # dot product and angle
        dot = torch.clamp((z0_norm * z1_norm).sum(), -1.0, 1.0)
        ...
        # stable slerp weights
        w0 = torch.sin((1 - t) * theta) / (sin_theta + eps)
        w1 = torch.sin(t * theta) / (sin_theta + eps)
        z_slerp = w0 * z0 + w1 * z1

        # fallback blend for small angles (near-linear regime)
        blend = (theta < 1e-3).to(z0.dtype).unsqueeze(-1)  # ensure broadcastable

        z_lerp = (1 - t) * z0 + t * z1

        z_interp = blend * z_lerp + (1 - blend) * z_slerp

        # expand to batch
        z_sem_interp = z_interp.unsqueeze(0).repeat(z_sem_origin_np.shape[0], 1)

        if add_noise_term:
            z_sem_interp = z_sem_interp + 0.1 * torch.randn_like(z_sem_interp)

        sample_interp = self.sample_fn(self.model,
            shape=(z_sem_origin_np.shape[0], gene_size),
